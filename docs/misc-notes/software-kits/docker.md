...
以上命令可以完成对指定镜像创建一个比较完备的容器，指定了容器名称、用户名称和组别、主机名、用户工作目录、共享主机网络和时区设置，并挂载了主机的一些目录。指定passwd和group文件的只读挂载可以避免--user使用用户(组)id进行新建容器时引发的找不到用户名和组名的问题。并且避免了默认root用户导致的主机端无法访问容器新建文件的问题。

除了`--network=host`外，也可以使用`-p <宿主机端口>:<容器端口>`来进行端口映射。

注意这样创建的用户没有root权限，可以用`docker exec -u root xx`来以root用户进入容器。如果需要，则不使用user参数，但存在容器创建文件是root，宿主机无法修改的问题。
