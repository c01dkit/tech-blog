...
可以使用[zerotier](https://www.zerotier.com/)，登录以后创建一个网络。然后需要组局域网的设备下载zerotier以后join上就行了。或者选择从源码自己编译：从[这里](https://github.com/zerotier/ZeroTierOne/releases)下载源码包，解压缩以后make即可（需要gcc/clang、rust编译环境，以及libssl-dev）
